...
function receiptTextLines(receipt: Awaited<ReturnType<typeof loadPrintableReceipt>>) {
    ...
    lines.push(
        ...
        ...pairLines("Payment Date", snapshot.paymentTransactionDate?.slice(0, 10) ?? formatDateTime(snapshot.paymentDateTime)).map((text) => ({ text })),
        ...pairLines("Entered", formatDateTime(snapshot.enteredAt ?? snapshot.paymentDateTime)).map((text) => ({ text })),
        ...(snapshot.isBackdated ? [{ bold: true, center: true, size: 10, text: "BACKDATED ADMIN ENTRY" }] : []),
        ...(snapshot.isBackdated && snapshot.backdatingReason ? pairLines("Reason", snapshot.backdatingReason).map((text) => ({ text })) : []),
    ...
    lines.push(
        ...
        ...(snapshot.isBackdated ? pairLines("Backdated By", safe(snapshot.preparedByName ?? snapshot.recordedByName, "Admin")).map((text) => ({ text })) : []),
