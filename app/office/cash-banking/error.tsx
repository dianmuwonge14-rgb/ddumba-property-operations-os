...
export default function CashBankingError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
    const reference = error.digest ?? "cash-banking-office";
    return (
        <main className="min-h-screen bg-slate-950 px-4 py-10 text-white">
            <section className="mx-auto max-w-3xl rounded-[28px] border border-red-300/20 bg-red-500/10 p-6 shadow-2xl shadow-black/25">
                <p className="text-xs font-black uppercase tracking-[0.22em] text-red-200">Cash Banking</p>
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                    ...
                    <a href="/office/dashboard" className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-black text-white">
                        Return to Dashboard
                    </a>
                </div>
            </section>
