-- Employee Management live schema completion.
-- Additive only. Supports the Admin Employee form without resetting employee data.
...
alter table public.employees
    ...
    add column if not exists daily_lunch_allowance numeric(14,2) not null default 0,
...
set
    salary_payment_day = coalesce(salary_payment_day, salary_receiving_day, 28),
    salary_receiving_day = coalesce(salary_receiving_day, salary_payment_day, 28),
    start_work_date = coalesce(start_work_date, hire_date),
    primary_office_id = coalesce(primary_office_id, office_id)
where salary_payment_day is null
   or salary_receiving_day is null
   or start_work_date is null
   or primary_office_id is null;

alter table public.employees
    alter column salary_payment_day set default 28,
    alter column salary_receiving_day set default 28;
...
begin
    ...
    if not exists (
        select 1 from pg_constraint
        where conname = 'employees_basic_salary_check'
          and conrelid = 'public.employees'::regclass
    ) then
        alter table public.employees
            add constraint employees_basic_salary_check check (basic_salary >= 0);
    end if;

    if not exists (
        select 1 from pg_constraint
        where conname = 'employees_daily_lunch_allowance_check'
          and conrelid = 'public.employees'::regclass
    ) then
        alter table public.employees
            add constraint employees_daily_lunch_allowance_check check (daily_lunch_allowance >= 0);
    end if;
    ...
    if not exists (
        select 1 from pg_constraint
        where conname = 'employees_salary_receiving_day_check'
          and conrelid = 'public.employees'::regclass
    ) then
        alter table public.employees
            add constraint employees_salary_receiving_day_check check (salary_receiving_day between 1 and 31);
    end if;
end $$;
...
create table if not exists public.employee_allowance_settings (
    ...
    daily_lunch_allowance numeric(14,2) not null default 0,
    salary_payment_day integer not null default 28,
...
create index if not exists idx_employees_company_office_status
    on public.employees(company_id, office_id, status);
...
create index if not exists idx_employee_allowance_settings_active
    on public.employee_allowance_settings(company_id, employee_id, active);
