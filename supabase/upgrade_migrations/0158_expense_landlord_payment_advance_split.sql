-- Phase 158: Expense-routed landlord payments with live payable/advance split.
-- Additive only. Does not reset landlord data, payments, expenses, or advances.
...
alter table public.landlord_payment_expense_requests
    ...
    add column if not exists normal_payment_amount numeric(14,2) not null default 0,
    add column if not exists advance_amount numeric(14,2) not null default 0,
    add column if not exists current_net_payable numeric(14,2) not null default 0,
    add column if not exists already_paid_amount numeric(14,2) not null default 0,
    ...
    add column if not exists active_advance_balance numeric(14,2) not null default 0,
    add column if not exists pending_request_amount numeric(14,2) not null default 0,
    add column if not exists flag_reason text,
    add column if not exists advance_agreement jsonb not null default '{}'::jsonb,
    add column if not exists approved_advance_id uuid references public.landlord_advances(id) on delete set null;
...
create index if not exists idx_landlord_payment_expense_requests_landlord
    on public.landlord_payment_expense_requests(company_id, landlord_id, payment_month, status);
...
using (
    ...
    or (
        landlord_payment_expense_requests.company_id = public.ddumba_v1_current_company_id()
...
using (
    public.ddumba_v1_can_access_entity(
        landlord_payment_expense_requests.company_id,
        landlord_payment_expense_requests.office_id
    )
)
with check (
    status = 'pending'
