-- Fast, compact tenant search for Payments Entry typing.
-- Additive only: no payment, tenant, balance, billing, or receipt data is changed.

create extension if not exists pg_trgm with schema extensions;

create index if not exists idx_payments_entry_rooms_room_trgm
    on public.rooms using gin (lower(room_number) gin_trgm_ops);

create index if not exists idx_payments_entry_rooms_active_office
    on public.rooms(company_id, office_id, status, id);

create index if not exists idx_payments_entry_tenants_name_trgm
    on public.tenants using gin (lower(full_name) gin_trgm_ops)
    where status = 'active';

create index if not exists idx_payments_entry_tenants_phone_digits_trgm
    on public.tenants using gin ((regexp_replace(coalesce(phone, ''), '\D', '', 'g')) gin_trgm_ops)
    where status = 'active';

create index if not exists idx_payments_entry_tenants_active_office
    on public.tenants(company_id, office_id, status, room_id, id)
    where status = 'active';
...
as $$
    ...
    candidate_rows as (
        select
            ...
            case
                ...
                when lower(trim(coalesce(t.full_name, ''))) like (select q from search_input) || '%' then 2
                ...
                when lower(trim(r.room_number)) like '%' || (select q from search_input) || '%' then 4
                when lower(trim(coalesce(t.full_name, ''))) like '%' || (select q from search_input) || '%' then 5
                when (select q_digits from search_input) <> '' and regexp_replace(coalesce(t.phone, ''), '\D', '', 'g') like '%' || (select q_digits from search_input) || '%' then 6
                else 20
            end as match_rank
        from public.rooms r
        ...
        order by match_rank, r.room_number, t.full_name
    ...
    order by r.match_rank, r.room_number, t.full_name;
$$;
