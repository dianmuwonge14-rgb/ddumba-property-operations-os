-- Employee All Rounder / All Offices assignment support.

alter table public.employees
    add column if not exists employee_assignment_type text not null default 'fixed_office',
    add column if not exists default_office_id uuid references public.offices(id) on delete set null;
...
begin
    if not exists (
        select 1
        from pg_constraint
        where conname = 'employees_assignment_type_check'
          and conrelid = 'public.employees'::regclass
    ) then
        alter table public.employees
            add constraint employees_assignment_type_check
            check (employee_assignment_type in ('fixed_office', 'all_rounder'));
    end if;
end $$;

update public.employees
set default_office_id = coalesce(default_office_id, office_id),
    employee_assignment_type = case
        when employee_assignment_type = 'all_rounder' then 'all_rounder'
        else 'fixed_office'
    end
where default_office_id is null
   or employee_assignment_type is null;

create index if not exists idx_employees_assignment_type
    on public.employees(company_id, employee_assignment_type, status);

create index if not exists idx_employees_default_office
    on public.employees(company_id, default_office_id);
...
begin
  ...
  where company_id = v_company_id
    ...
    and (
      ...
      or employee_assignment_type = 'all_rounder'
    )
  order by
    case when office_id = p_office_id then 0 else 1 end,
    created_at asc
  limit 1;
  ...
  if p_event_type = 'check_in' then
    ...
    values (
      ...
      'not_checked_out',
      'Africa/Kampala',
      jsonb_build_object('source', 'web', 'assignment_type', coalesce(v_employee.employee_assignment_type, 'fixed_office')),
      now(),
      now()
    )
    ...
    do update set
      ...
      session_info = coalesce(public.office_daily_attendance.session_info, '{}'::jsonb) || excluded.session_info,
      updated_at = now()
  ...
  values (
    ...
    'attendance_event',
    v_event.id,
    jsonb_build_object('event', to_jsonb(v_event), 'daily_attendance', to_jsonb(v_daily), 'employee_assignment_type', coalesce(v_employee.employee_assignment_type, 'fixed_office')),
    now()
  );
