                    ...
                    {visible.length ? visible.map((receipt) => (
                        <article key={receipt.id} className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                                <button type="button" onClick={() => { void logReceiptPrintOrDownload({ channel: "print", receiptId: receipt.id }); window.print(); }} className="inline-flex items-center gap-1 rounded-xl bg-slate-950 px-3 py-2 text-xs font-black text-white"><Printer size={13} /> Reprint</button>
                                <button type="button" onClick={() => { void logReceiptPrintOrDownload({ channel: "download_pdf", receiptId: receipt.id }); window.print(); }} className="inline-flex items-center gap-1 rounded-xl bg-blue-700 px-3 py-2 text-xs font-black text-white"><Download size={13} /> PDF</button>
